...
**在线应用：** [ashare-quant-app-22rxjfpcsjxzt67wu9lbtv.streamlit.app](https://ashare-quant-app-22rxjfpcsjxzt67wu9lbtv.streamlit.app/)

**GitHub 仓库：** [SHG4666/ashare-quant-app](https://github.com/SHG4666/ashare-quant-app)
...
- Python version：使用 Streamlit Community Cloud 当前默认版本
...
- 历史行情始终优先请求在线数据源，在线源全部失败时才使用静态备份。
- 静态备份优先来自 baostock；若其更新滞后，则使用东方财富前复权日线补位，并与最新市场行情核对日期和收盘价。
- GitHub Actions 在交易日收盘后更新两次；单只股票失败时保留其上次有效文件，不再阻断其他股票更新。
- 全市场自动选股在没有本地数据库时使用候选快照。
- 页面会显示数据来源和截止日期；静态备份明确标注为“非实时”。
...
该命令需要 baostock、东方财富/腾讯行情网络访问以及本机 Sequoia-X 数据库（用于更新全市场候选快照）。
...
│   ├── watchlist_history_manifest.json
│   └── watchlist_history/
├── data_cache/
│   └── watchlist.txt
├── scripts/
